...
public class MarksheetModel {
	...
	public List search(MarksheetBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getResult() != null && bean.getResult().length() > 0 && bean.getResult().equalsIgnoreCase("Pass")) {
				 System.out.println("Inner" +bean.getResult());
				sql.append(" AND physics>=33 AND chemistry>=33 AND maths>=33");
			} else if (bean.getResult() != null && bean.getResult().length() > 0 && bean.getResult().equalsIgnoreCase("Fail")) {
